...
import model.*;
import model.exceptions.BetException;
...
public class Viewer {
    ...
    public void toBet(){
        ...
        if(sc.hasNextInt()){
            ...
            try {
                if(lotController.isOpened(lot)){
                    System.out.println("Введите вашу цену: ");
                    if(sc.hasNextInt()){
                        try {
                            userAnswer = sc.nextInt();
                            DataHolder.getDataHolder().getBets().add(new Bet(DataHolder.getDataHolder().getBets().size() + 1,
                                    currentUser.getId(), lot.getId(), userAnswer));
